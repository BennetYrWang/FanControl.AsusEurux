...
- 提供四个 RPM 传感器：被 FanControl 控制的端口按目标 PWM 显示标定估算值，未被控制的端口
  继续显示控制器返回的原始 RPM。
- 在 FanControl 中提供四个 0–100% 控制项，并与 RPM 传感器自动配对。
- 由于 USB 固件 0.6 不会把协议值 0 解释为停转，FanControl 的逻辑 0% 会映射为协议值 1。
- 修改一路时保留其余三路的目标值。
...
- 在已测试的 USB 固件 0.6 上，协议值 `0` 是特殊值且不会让风扇停转。因此 FanControl 的 0%
  会发送为协议值 `1`；原始诊断探针的行为保持不变。
- 端口受控时显示的是估算 RPM：低于 5% 显示 0 RPM，5% 显示 300 RPM，100% 显示
  2250 RPM，中间线性插值。该估算值无法检测风扇堵转或断开。
